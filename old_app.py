...
# Load existing data from the JSON file or initialize an empty dictionary
if os.path.exists(STUDENTS_JSON_PATH):
    with open(STUDENTS_JSON_PATH, "r") as file:
...
def login():
    ...
    hashed_password_from_database = students_db[email]['password']
...
def forgot_password():
    if request.method == 'POST':
        ...
        students_db[email]['password'] = hashed_password.decode('utf-8')
...
def testsheet():
    ...
    if (session.get('flashcards_completed_arithematic') and session.get('flashcards_completed_number_series') and 
